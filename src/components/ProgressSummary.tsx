...
const ProgressSummary: React.FC<ProgressSummaryProps> = ({ skillPath }) => {
    ...
    <div className="bg-white rounded-lg shadow p-4 mb-6">
      ...
      <div className="mb-4">
        <div className="flex justify-between mb-1">
          <span className="text-sm font-medium">Pontos: {points}/{maxPoints}</span>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="border rounded-md p-3 bg-gray-50">
          <div className="text-sm font-medium mb-1 text-gray-500">Nível Atual</div>
          {currentLevel ? (
            <div className="flex items-center">
              <TagIcon className="h-4 w-4 mr-1 text-green-600" />
              <span className="font-bold">{currentLevel.name}</span>
              <span className="text-xs ml-2 text-gray-600">({currentLevel.level})</span>
            </div>
          ) : (
            <div className="text-sm text-gray-500">Não avaliado</div>
          )}
        </div>
        
        <div className="border rounded-md p-3 bg-gray-50">
          <div className="text-sm font-medium mb-1 text-gray-500">Próximo Nível</div>
          {nextLevel ? (
            <div className="flex items-center">
            ...
            <div className="text-sm text-green-600 font-medium">Nível máximo alcançado!</div>
          )}
        </div>
      </div>
    </div>
