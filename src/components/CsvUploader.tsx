...
const CsvUploader: React.FC<CsvUploaderProps> = ({ onImport, onClose }) => {
  ...
  const handleFileChange = async (
    ...
    try {
      ...
      toast.error(
        "Falha ao processar arquivo CSV. Por favor, verifique o formato e tente novamente."
      );
    } finally {
      setIsLoading(false);
  ...
  const readFileAsText = async (file: File): Promise<string> => {
    ...
    const encodings = ["iso-8859-1", "utf-8", "windows-1252", "latin1"];

    // First pass - try to detect encoding
    let bestEncoding = "iso-8859-1"; // Default
    let bestScore = 0;
    ...
    <div className="space-y-4">
      <div className="space-y-2">
        <Label htmlFor="csv-file">
          Importar Trilha de Carreira a partir de CSV
        </Label>
        ...
        <p className="text-sm text-gray-500">
          Faça upload de um arquivo CSV com dados da trilha de carreira. O
          arquivo deve ter o formato esperado com colunas: career,
          behaviorDifferentiator, baseBehavior, level, track, groupCompetence,
